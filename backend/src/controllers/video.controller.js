...
const updateVideo = asyncHandler(async (req, res) => {
  ...
  if (Object.keys(req.files).length > 0) {
    ...
    if (req.files.file[0].mimetype.split("/")[0] === "video") {
      ...
      req.files.file[0].mimetype.split("/")[0] === "video" &&
      ...
      updateObject.videoFile = {
        ...
        public_id: newVideoFile.public_id,
      };
      updateObject.thumbnail = {
...
const addAndRemoveLikes = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, "Video likes updated successfully", updatedVideo)
    );
});
